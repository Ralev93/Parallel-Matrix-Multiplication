...
public class Options {
	...
	public void parseOptions(String[] options) throws InvalidOptionException {
		for (int i = 0; i < options.length; i++) {
			switch (options[i]) {
			case "-m":
				m = Integer.parseInt(options[++i]);
				useFile = false;
				break;
			case "-n":
				n = Integer.parseInt(options[++i]);
				useFile = false;
				break;
			case "-k":
				k = Integer.parseInt(options[++i]);
				useFile = false;
				break;
			case "-i":
		...
		if (!showGUI && !useFile && (m <= 0 || n <= 0 || k <= 0))
			throw new InvalidOptionException(
					"Incorrect dimensions of the matrices to multiply.");
